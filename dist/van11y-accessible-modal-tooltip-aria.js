...
(function (doc) {
  ...
  var createModalTooltip = function createModalTooltip(config) {
    ...
    var buttonCloseInner = config.modalTooltipCloseImgPath ? "<img src=\"".concat(config.modalTooltipCloseImgPath, "\" alt=\"").concat(config.modalTooltipCloseText, "\" class=\"").concat(MODAL_TOOLTIP_CLOSE_IMG_CLASS_SUFFIX, "\" />") : "<span class=\"".concat(MODAL_TOOLTIP_CLOSE_TEXT_CLASS_SUFFIX, "\">\n              ").concat(config.modalTooltipCloseText, "\n             </span>");
    ...
    var title = config.modalTooltipTitle !== '' ? "<h1 id=\"".concat(MODAL_TOOLTIP_TITLE_ID, "\" class=\"").concat(titleClassName, "\">\n               ").concat(config.modalTooltipTitle, "\n             </h1>") : '';
    var button_close = "<button type=\"button\" class=\"".concat(MODAL_TOOLTIP_BUTTON_JS_CLASS, " ").concat(buttonCloseClassName, "\" id=\"").concat(MODAL_TOOLTIP_BUTTON_JS_ID, "\" title=\"").concat(config.modalTooltipCloseTitle, "\" ").concat(MODAL_TOOLTIP_BUTTON_CONTENT_BACK_ID, "=\"").concat(config.modalTooltipContentId, "\" ").concat(MODAL_TOOLTIP_BUTTON_FOCUS_BACK_ID, "=\"").concat(config.modalTooltipFocusBackId, "\">\n                             ").concat(buttonCloseInner, "\n                            </button>");
    ...
    if (content === '' && config.modalTooltipContentId) {
      ...
      if (contentFromId) {
        content = "<div id=\"".concat(MODAL_TOOLTIP_CONTENT_JS_ID, "\">\n                            ").concat(contentFromId.innerHTML, "\n                           </div");
        // we remove content from its source to avoid id duplicates, etc.
        contentFromId.innerHTML = '';
      }
    } else {
      // we put it in a p
      content = "<".concat(MODAL_TOOLTIP_CONTENT_TEXT_ONLY_WRAPPER, ">\n                            ").concat(content, "\n                       </").concat(MODAL_TOOLTIP_CONTENT_TEXT_ONLY_WRAPPER, ">");
    }
    return "<dialog id=\"".concat(id, "\" class=\"").concat(modalTooltipClassName, " ").concat(MODAL_TOOLTIP_DIALOG_JS_CLASS, "\" ").concat(ATTR_ROLE, "=\"").concat(MODAL_TOOLTIP_ROLE, "\" ").concat(ATTR_OPEN, " ").concat(ATTR_LABELLEDBY, "=\"").concat(MODAL_TOOLTIP_TITLE_ID, "\">\n                  <div role=\"document\" class=\"").concat(modalTooltipClassWrapper, "\">\n                    ").concat(button_close, "\n                    <div class=\"").concat(contentClassName, "\">\n                      ").concat(title, "\n                      ").concat(content, "\n                    </div>\n                  </div>\n                </dialog>");
  };
      ...
      ['click', 'keydown'].forEach(function (eventName) {
        doc.body.addEventListener(eventName, function (e) {
          ...
          if ((e.target.getAttribute('id') === MODAL_TOOLTIP_BUTTON_JS_ID || parentButton !== '') && eventName === 'click') {
            var _modalTooltip2 = findById(MODAL_TOOLTIP_DIALOG_JS_ID);
            var _modalTooltipContent = findById(MODAL_TOOLTIP_CONTENT_JS_ID) ? findById(MODAL_TOOLTIP_CONTENT_JS_ID).innerHTML : '';
            var _modalTooltipButtonClose = findById(MODAL_TOOLTIP_BUTTON_JS_ID);
            var _modalTooltipFocusBackId = _modalTooltipButtonClose.getAttribute(MODAL_TOOLTIP_BUTTON_FOCUS_BACK_ID);
            var _modalTooltipLauncher2 = findById(_modalTooltipFocusBackId);
            var _contentBackId = _modalTooltipButtonClose.getAttribute(MODAL_TOOLTIP_BUTTON_CONTENT_BACK_ID);
            closeModalTooltip({
              modalTooltip: _modalTooltip2,
              modalTooltipContent: _modalTooltipContent,
              modalTooltipFocusBackId: _modalTooltipFocusBackId,
              contentBackId: _contentBackId,
              fromId: e.target.getAttribute('id')
            });

            // remove class is-active on launcher and give focus to it
            removeClass(_modalTooltipLauncher2, MODAL_TOOLTIP_IS_ACTIVE_CLASS);
            _modalTooltipLauncher2.focus();
          }
          ...
          if (modalTooltip && eventName === 'click' &&
          // click anywhere with a modal tooltip opened
          e.target.getAttribute('id') !== MODAL_TOOLTIP_DIALOG_JS_ID && parentModalTooltip === '' // not a click in modal tooltip
          ) {
            var _modalTooltipButtonClose2 = findById(MODAL_TOOLTIP_BUTTON_JS_ID);
            _modalTooltipButtonClose2.click();
          ...
          if (modalTooltip && eventName === 'keydown') {
            var _modalTooltipContent2 = findById(MODAL_TOOLTIP_CONTENT_JS_ID) ? findById(MODAL_TOOLTIP_CONTENT_JS_ID).innerHTML : '';
            var _modalTooltipButtonClose3 = findById(MODAL_TOOLTIP_BUTTON_JS_ID);
            var _modalTooltipFocusBackId2 = _modalTooltipButtonClose3.getAttribute(MODAL_TOOLTIP_BUTTON_FOCUS_BACK_ID);
            var _contentBackId2 = _modalTooltipButtonClose3.getAttribute(MODAL_TOOLTIP_BUTTON_CONTENT_BACK_ID);
            var _modalTooltipLauncher3 = findById(_modalTooltipFocusBackId2);
            ...
            if (e.keyCode === 27) {
              closeModalTooltip({
                ...
                modalTooltipContent: _modalTooltipContent2,
                modalTooltipFocusBackId: _modalTooltipFocusBackId2,
                contentBackId: _contentBackId2
              });

              // remove class is-active on launcher and give focus to it
              removeClass(_modalTooltipLauncher3, MODAL_TOOLTIP_IS_ACTIVE_CLASS);
              _modalTooltipLauncher3.focus();
            }
